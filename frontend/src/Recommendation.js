...
function Recommendation() {
    ...
    const getRecommendations = async () => {
        try {
            ...
            const response = await axios.get(`https://movie-recommendation-backen.onrender.com/api/recommend?title=${movie}`);
    ...
    const fetchSuggestions = async (query) => {
        try {
            if (query.length > 0) {
                const response = await axios.get(`https://movie-recommendation-backen.onrender.com/api/suggestions?query=${query}`);
